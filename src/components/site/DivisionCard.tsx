...
type DivisionStatus = "active" | "development" | "strategic";
...
export function DivisionCard({
  ...
  const label =
    status === "active" ? "Active Operation" : status === "development" ? "In Development" : "Strategic Division";

  return (
    <Link to="/businesses/$slug" params={{ slug }} className="group block">
      <article className="relative overflow-hidden bg-navy-deep h-[440px] cursor-pointer">
        <img
          src={image}
          alt={name}
          loading="lazy"
        ...
        <div className="relative h-full flex flex-col justify-end p-8 text-white">
          {status && <div className="text-[10px] uppercase tracking-[0.3em] text-gold mb-4">{label}</div>}
